...
### Provider clinical requests — current implementation status (2026-09-23)

The older phase-status paragraph above predates provider request APIs. Provider prescription creation/approval and lab creation now have HTTP routes, scoped list/detail reads, and write idempotency. Provider prescriptions are separate from pharmacy fulfillment: only an approved `ACCEPTED` prescription may be submitted to `POST /v1/pharmacy-orders/provider`, which writes into the existing `PharmacyOrder` broadcast queue. Provider lab requests enter the existing `LabOrder` branch queue from an uploaded `LAB_REFERRAL`; no test catalog or test-selection endpoint is part of the product (user decision, 2026-09-25). Notification templates consume transactional outbox events. Do not treat this paragraph as an end-to-end release sign-off: local migration deployment, dashboard/mobile verification, and broader acceptance scenarios remain explicit gates in Part 51.10.
