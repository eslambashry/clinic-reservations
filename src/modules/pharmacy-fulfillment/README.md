...
**2026-09-24 — clinic delivery semantics and OCR placeholder removal.** `CLINIC_HANDOVER` means the pharmacy sends the order to a clinic; like `DELIVERY`, it requires a delivery-capable pharmacy branch, transitions to `OUT_FOR_DELIVERY`, and is audited as a courier handoff. The Flutter and pharmacy staff console labels/filters reflect this distinction. The current delivery MVP still has no courier integration or persisted clinic destination/appointment association, so this status does not itself dispatch a courier or identify a clinic address. The no-op OCR extractor now returns no suggested items rather than persisting a fabricated drug name; pharmacists price the uploaded prescription image as before. Historical placeholder rows are hidden in the provider card.
