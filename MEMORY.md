...
| `DEC-001` | Payment gateway = **Paymob** (File 10 Part 10's recommendation, implemented 2026-09-05 per File 12 Part 50 — Payments Phase 9). `payments/infrastructure/paymob-payment-gateway.adapter.ts` behind `PaymentGatewayPort`; `PAYMOB_*` env vars stay optional (credentials not provisioned yet) — the adapter throws `PAYMENT_GATEWAY_NOT_CONFIGURED` at call time rather than blocking boot. |
| `DEC-B02`/`DEC-B03`/`DEC-B06`/`DEC-B07` | **OPEN** — secrets manager, WAF, cross-region backup, upload virus scanning. Not needed until the phase that requires them; do not pre-integrate speculatively. |
...
- **Concurrency-critical paths** (read File 11 Part 11 before touching):
  ...
  - Webhook idempotency: insert into `webhook_events` (unique `idempotency_key`) **before** any side effect; unique-constraint failure means "already processed," not an error. Generic `WebhookEventRepository` lives in `shared/core/webhooks/` (global module, same pattern as `PolicyConfigReader`) — reuse it for any future webhook, don't build a second mechanism.
